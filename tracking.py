...
while True:
  ...
  avg_height_range = np.mean(top_bottom_list, axis=0).astype(np.int) #평균을 내는 것임. 탑과 바텀의 평균을 정수로
  avg_width_range = np.mean(left_right_list, axis=0).astype(np.int) #좌측과 우측의 평균을 정수로
  ...
  if fit_to == 'width': #는 무조건 height
    ...
    avg_height_range = avg_height_range.astype(np.int).clip(0, 9999) #간단하게 삑사리 방지용으로 0이하값은 0으로 9999이상값은 9999로 min하고 max를 정해두는 것임. 범위 벗어나지 않게 하려는 것.
    ...
    ]).astype(np.int).clip(0, 9999) #에서 삑사리 방지.
